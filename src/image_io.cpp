...
{
    const int srcPitchY = width;
    const int srcPitchU = width;
    const int srcPitchV = width;

    const uint8_t* i444_p0 = i444;
    const uint8_t* i444_p1 = i444 + width * height;
    const uint8_t* i444_p2 = i444 + width * height * 2;

    const int dstPitch = width * 3;
    for (int i = 0; i < height; i++)
    {
        const uint8_t* pY = i444_p0 + i * srcPitchY;
        const uint8_t* pU = i444_p1 + i * srcPitchU;
        const uint8_t* pV = i444_p2 + i * srcPitchV;
        uint8_t* prgb = rgb + i * dstPitch;
        for (int j = 0; j < width; j++)
        {
            uint8_t y, u, v, r, g, b;
